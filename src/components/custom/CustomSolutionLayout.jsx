...
import { useAutomationId } from '../../hooks/useAutomationId';
...
const getNavItems = (features) => {
  ...
  return [
    ...
    { id: 'errors', label: 'Баги и ошибки', icon: ActivityIcon, path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_ERRORS() },
    {
      ...
      label: 'Аккаунты',
      icon: UsersIcon,
      path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_ACCOUNTS(),
      hidden: isDmpBot,
    },
    {
      id: 'chats',
      label: 'Чаты',
      icon: ChatIcon,
      path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_CHATS(),
      hidden: isDmpBot,
    },
    { id: 'leads', label: 'Лиды', icon: LeadsIcon, path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_LEADS() },
    {
      id: 'prompts',
      label: 'Промпты',
      icon: PromptIcon,
      path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_PROMPTS(),
      hidden: isDmpBot,
    },
    {
      id: 'dmp',
      label: 'DMP.one',
      icon: PlugIcon,
      path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_DMP(),
    ...
    },
    {
      ...
      label: 'AmoCRM',
      icon: PlugIcon,
      path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_AMOCRM(),
      hidden: isDmpBot || !features?.is_amocrm_enabled,
    },
    { id: 'settings', label: 'Настройки', icon: SettingsIcon, path: NAVIGATION_ROUTES.CUSTOM_AUTOMATION_SETTINGS() },
  ];
};
...
const CustomSolutionLayout = () => {
  ...
  const { isAuthenticated, logout } = useCustomAuth();
  const id = useAutomationId();
  const [features, setFeatures] = useState({});
  const [title, setTitle] = useState('Кабинет');
  ...
  useEffect(() => {
    ...
    customService
      ...
      .then((data) => {
        ...
        setFeatures(data || {});
        setTitle(data?.name || data?.client_name || 'Кабинет');
      })
      .catch(() => {});
    return () => {
      mounted = false;
  ...
  const navItems = getNavItems(features).filter((item) => !item.hidden);
  const isActive = (path) => location.pathname === path || location.pathname.startsWith(`${path}/`);

  const handleLogout = () => {
    logout();
    navigate(NAVIGATION_ROUTES.CUSTOM_LOGIN, { replace: true });
  };
    ...
    <div className="project-layout">
      <header className="project-topbar">
        ...
        <div className="project-topbar-right">
          <span className="project-topbar-user">Клиент</span>
          <button type="button" className="project-topbar-back" onClick={handleLogout}>
            Выйти
          </button>
          <button
            type="button"
